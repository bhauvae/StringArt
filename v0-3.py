...
def image_preprocessing(image_path, output_path="temp.png", max_length=512):
...
def generate_image(
    ...
    def string_radon_transform(alpha_string, distance_string):
    
        # === CONFIGURATION ===
        canvas_size = gray.shape  # Image dimensions: canvas_size x canvas_size
        theta1_deg = alpha_string - np.rad2deg(
            np.arccos(distance_string / radius)
        )  # Angle for line start (degrees)
        theta2_deg = alpha_string + np.rad2deg(
            np.arccos(distance_string / radius)
        )  # Angle for line end (degrees)

        # === SETUP BLACK CANVAS ===
        canvas = np.zeros(canvas_size, dtype=np.uint8)

        # === COMPUTE LINE ENDPOINTS BASED ON ANGLES ===
        theta1_rad = np.deg2rad(theta1_deg)
        theta2_rad = np.deg2rad(theta2_deg)

        x1 = int(center[0] + radius * np.cos(theta1_rad))
        y1 = int(center[1] + radius * np.sin(theta1_rad))
        x2 = int(center[0] + radius * np.cos(theta2_rad))
        y2 = int(center[1] + radius * np.sin(theta2_rad))

        # === DRAW WHITE LINE ===
        cv2.line(canvas, (x1, y1), (x2, y2), color=255, thickness=1)

        # === RADON TRANSFORM ===
        # Convert to float for Radon (scikit-image expects float image)
        canvas_float = canvas.astype(np.float32)
        theta_values = alphas
        radon_img = radon(canvas_float, theta=theta_values, circle=True)

        
        return radon_img, theta1_rad, theta2_rad
...
def main(input_image="img2.jpeg", output_art="string-art2.png"):
    ...
    generate_image(input_preprocessed="temp.png", output_path=output_art, string_count=5)
